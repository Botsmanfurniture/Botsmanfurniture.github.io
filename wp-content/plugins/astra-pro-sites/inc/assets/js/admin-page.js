...
(function($){
	...
	AstraSitesAdmin = {
		...
		_renderDemoPreview: function(anchor) {
			...
			// Update required plugins list for premium sites
			if ( demoType === 'premium' ) {
				$.ajax({
					url  : astraSitesAdmin.ajaxurl,
					type : 'POST',
				...
				.done(function ( demo_data ) {
					if( false !== demo_data.success ) {
						templateData[0].required_plugins = JSON.stringify( demo_data.data['required-plugins'] );
					}
				});
				requiredPlugins = JSON.parse( templateData[0].required_plugins );
				console.log( 'Required plugins list updated:' );
